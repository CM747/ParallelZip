...
void *RLEzip(void *param){
    ...
    fgets(version, sizeof(version), iofiles->fin);
    fscanf(iofiles->fin, "%d ", &col);
    fscanf(iofiles->fin, "%d", &row);
    fscanf(iofiles->fin, "%d", &max_gray);

    fwrite(&col, sizeof(int), 1, iofiles->fout);
    fwrite(&row, sizeof(int), 1, iofiles->fout);


    for (int i=0; i<row; i++) {
        fscanf(iofiles->fin, "%d ", &curr);
        count = 1;
        for (int j=1; j<col; j++) {
            fscanf(iofiles->fin, "%d ", &c);
            if(c==curr){
                count += 1;
            }else{
                c1 = (char)curr;
                fwrite(&count, sizeof(int), 1, iofiles->fout);
                fwrite(&c1, sizeof(char), 1, iofiles->fout);
                curr = c;
                count = 1;
            }
        }
